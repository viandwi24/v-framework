...
class api {    
	...
	private static function set_route($method, $args)
	{
		...
		$args[1] = self::clean_url($args[1]);


		if (self::getIstance()::$temp_prefix != null) {
			$args[1] = self::getIstance()::$temp_prefix.$args[1];
		}
		$args[1] = self::create_pattern($args[1]);
		$args[1] = self::clean_url( $args[1] );
		
		array_push(self::getIstance()::$route[$method], [$args[1], $args[2], '', []]);
	}
	...
	public static function api()
	{
		...
		self::getIstance()::set_route($args[0], $args);
		return new static;
	}
	...
	private static function search($method)
	{
		...
		foreach ($route as $key => $value) {
			...
			if ($sama)
			{
				...
				self::getIstance()::$route_api = true;
				return true;
				break;
			}
		}
